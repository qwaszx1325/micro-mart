...
func (pc *PermissionCreate) SetAllowed(b bool) *PermissionCreate {
	pc.mutation.SetAllowed(b)
	return pc
}

// SetNillableAllowed sets the "allowed" field if the given value is not nil.
func (pc *PermissionCreate) SetNillableAllowed(b *bool) *PermissionCreate {
	if b != nil {
		pc.SetAllowed(*b)
	}
	return pc
...
func (pc *PermissionCreate) defaults() {
	if _, ok := pc.mutation.Allowed(); !ok {
		v := permission.DefaultAllowed
		pc.mutation.SetAllowed(v)
	}
...
func (pc *PermissionCreate) check() error {
	...
	if _, ok := pc.mutation.Allowed(); !ok {
		return &ValidationError{Name: "allowed", err: errors.New(`ent: missing required field "Permission.allowed"`)}
	}
	return nil
}
...
func (pc *PermissionCreate) createSpec() (*Permission, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.Allowed(); ok {
